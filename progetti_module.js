...
// Diciannovesima tornata: Giovanni segnalava di non vedere "nessun
// pulsante" — il vecchio pulsante esisteva già (con la pila di 20 azioni,
// vedi _UNDO_MAX_AZIONI sopra) ma viveva tra le "schede" (.fonte-tab) del
// pannello sinistro con stile pillola disabilitata (opacità .4), facile da
// confondere con un'etichetta e da non notare affatto. Ora è un pulsante
// vero (.btn) nella barra in alto, sempre nello stesso punto, con un
// contatore visibile di quante azioni si possono ancora annullare (fino a
// _UNDO_MAX_AZIONI = 20) invece del solo tooltip.
function _aggiornaStatoBottoneUndo() {
  const btn = document.getElementById('btn-annulla-progetto');
  if (!btn) return;
  const n = PROGETTI_STATE.cronologiaUndo.length;
  const azione = PROGETTI_STATE.cronologiaUndo[n - 1];
  btn.disabled = !azione;
  btn.textContent = n ? `↶ Annulla (${n}/${_UNDO_MAX_AZIONI})` : '↶ Annulla';
  btn.title = azione ? `Annulla: ${azione.etichetta} (Ctrl+Z)` : 'Niente da annullare (Ctrl+Z)';
}
  ...
  page.innerHTML = `
    <div class="page-header">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
        ...
        <button class="btn btn-sm" onclick="mostraFormCapitoloProgetto()">+ Gruppo/Capitolo/Paragrafo</button>
        <button id="btn-annulla-progetto" class="btn btn-sm" onclick="annullaUltimaModificaProgetto()" title="Niente da annullare (Ctrl+Z)" disabled>↶ Annulla</button>
    ...
    <div id="form-voce-manuale-progetto" class="card" style="display:none;margin-bottom:14px;flex-shrink:0">
      ...
      <div style="font-size:11px;color:var(--ink4);margin-top:6px">Il campo "Capitolo" qui sopra è solo per l'esportazione Excel/PDF (raggruppa le voci in colonna); per organizzare la vista a schermo in Gruppo/Capitolo/Paragrafo usa "+ Gruppo/Capitolo/Paragrafo" qui sopra.</div>
    </div>

    <div id="form-capitolo-progetto" class="card" style="display:none;margin-bottom:14px;flex-shrink:0">
      <div class="form-row">
        <select id="pc-livello" style="max-width:160px">
          <option value="1">Gruppo</option>
          <option value="2">Capitolo</option>
          <option value="3" selected>Paragrafo</option>
        </select>
        <input id="pc-titolo" placeholder="Titolo (es. Condominio / Appartamento / Impianto illuminazione...)" style="flex:1;min-width:260px">
      ...
      <div style="font-size:11px;color:var(--ink4);margin-top:6px">Crea una riga di struttura a 3 livelli (es. Gruppo = Condominio → Capitolo = Appartamento → Paragrafo = Impianto illuminazione). Le voci aggiunte dopo, da qui o dal pannello di destra, compariranno sotto di essa nell'ordine in cui vengono create.</div>
    </div>
...
// Diciannovesima tornata: rientro visivo della scaletta Gruppo/Capitolo/
// Paragrafo (vedi salvaCapitoloProgetto). Calcolato UNA VOLTA sull'elenco
// COMPLETO e non filtrato (PROGETTI_STATE.voci, già ordinato per "ordine")
// così il rientro di ogni riga resta corretto anche quando una ricerca
// nasconde alcune righe di mezzo — non lo si può ricalcolare scorrendo solo
// il sottoinsieme filtrato, altrimenti il livello "corrente" si perderebbe.
// Una riga titolo prende come rientro il proprio livello (1=Gruppo → nessun
// rientro, 2=Capitolo → un passo, 3=Paragrafo → due passi); una riga voce
// prende il rientro della struttura più recente sopra di lei (uno in più,
// per comparire visivamente "dentro"), o nessun rientro se non c'è ancora
// nessuna struttura sopra di lei. Le righe titolo "vecchie" (create prima di
// questa modifica, o agganciate automaticamente da aggiungiVoceAlProgetto
// dalla gerarchia del magazzino di provenienza) non hanno un campo
// "livello": trattate come livello 1, così restano visivamente come prima
// di questa modifica — nessuna regressione per i progetti già in uso.
function _calcolaLivelliAreaLavoro(vociComplete) {
  const mappa = new Map();
  let livelloCorrente = 0;
  vociComplete.forEach(v => {
    if (v.tipo === 'titolo') {
      const livello = v.livello || 1;
      mappa.set(v.id, 18 * (livello - 1));
      livelloCorrente = livello;
    } else {
      mappa.set(v.id, 18 * livelloCorrente);
    }
  });
  return mappa;
}
...
function renderAreaLavoro() {
  ...
  const rientri = _calcolaLivelliAreaLavoro(PROGETTI_STATE.voci);
  ...
  const righe = voci.map(v => {
    const rientro = rientri.get(v.id) || 0;
    ...
    if (v.tipo === 'titolo') {
      const etichettaLivello = _LIVELLO_STRUTTURA_LABEL[v.livello] ? `<span style="font-size:10px;font-weight:600;color:var(--ink4);text-transform:uppercase;letter-spacing:.04em;margin-right:6px">${_LIVELLO_STRUTTURA_LABEL[v.livello]}</span>` : '';
      return `
        <div class="albero-riga albero-riga-progetto albero-riga-progetto-titolo">
          ...
          <span></span>
          <span class="progetto-titolo-riga" style="padding-left:${rientro}px">${etichettaLivello}${escapeHtml(v.descrizione || '')}</span>
    ...
    const cellaDescrizione = inModifica
      ? `<textarea class="edit-textarea" style="margin-left:${rientro}px;width:calc(100% - ${rientro}px)" onchange="aggiornaCampoVoceProgetto('${v.id}', 'descrizione', this.value)">${escapeHtml(v.descrizione || '')}</textarea>`
      : `<span style="padding-left:${rientro}px;display:inline-block">${escapeHtml(v.descrizione || '')}</span>`;
...
// ─── Gruppi, capitoli e paragrafi veri (diciottesima tornata, esteso a 3
// livelli nella diciannovesima) ───
// Un gruppo/capitolo/paragrafo creato a mano è una riga "titolo" a tutti gli
// effetti (stessa forma e stesso rendering non modificabile/non conteggiato
// delle righe titolo agganciate in automatico da aggiungiVoceAlProgetto —
// vedi renderAreaLavoro), solo con fonte 'manuale' e nessun fonteNumero:
// serve a strutturare visivamente l'area di lavoro in una scaletta a 3
// livelli — Gruppo (1) > Capitolo (2) > Paragrafo (3), esempio pratico di
// Giovanni: Gruppo="Condominio" > Capitolo="Appartamento" >
// Paragrafo="Impianto illuminazione" — nell'ordine in cui le righe vengono
// create: le voci aggiunte dopo (da qui o dal pannello destro) compariranno
// sotto l'ultima riga di struttura creata, con un rientro visivo pari al
// livello della struttura sotto cui si trovano (vedi
// _calcolaLivelliAreaLavoro in renderAreaLavoro). Non è un vero albero con
// riferimenti espliciti padre/figlio: come il resto dell'area di lavoro
// (elenco piatto ordinato per "ordine"), l'appartenenza è dedotta dalla
// sequenza — la riga di struttura più recente "prima" di una voce, scorrendo
// dall'alto, è il suo genitore visivo.
//
// Indipendente dal campo di testo libero "Capitolo" già esistente per
// l'export (Excel/PDF/XPWE, vedi _raggruppaPerCapitolo): quel campo resta
// invariato e continua a raggruppare l'esportazione per il testo scritto
// voce per voce, senza nessun legame con questa struttura a schermo.
function mostraFormCapitoloProgetto() {
  document.getElementById('form-capitolo-progetto').style.display = 'block';
}

const _LIVELLO_STRUTTURA_LABEL = { 1: 'Gruppo', 2: 'Capitolo', 3: 'Paragrafo' };

async function salvaCapitoloProgetto() {
  const inp = document.getElementById('pc-titolo');
  const selLivello = document.getElementById('pc-livello');
  const titolo = inp.value.trim();
  const livello = parseInt(selLivello.value, 10) || 1;
  ...
  try {
    const ref = await db.collection(PROGETTI_STATE.collName).doc(PROGETTI_STATE.progettoAttivo.id).collection('voci').add(dato);
    _registraAzioneUndo(`Aggiunta ${_LIVELLO_STRUTTURA_LABEL[livello] || 'capitolo'}`, [{ tipo: 'elimina', id: ref.id }]);
    inp.value = '';
    selLivello.value = '3';
