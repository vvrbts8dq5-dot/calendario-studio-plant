...
//
// Diciannovesima tornata: stesso miglioramento fatto in prezzario_module.js
// — testo più esplicito su cosa significa il messaggio + pulsante "No,
// ignora" per chiuderlo senza migrare (scelta ricordata sul dispositivo via
// localStorage, i vecchi dati restano intatti finché non si preme "Migra
// dati esistenti").
  ...
  try {
    ...
    wrap.innerHTML = `
      <div class="import-banner" style="display:flex;margin-bottom:18px">
        <span>Analisi Prezzi aveva un formato più vecchio, senza annate: ho trovato voci rimaste in quella vecchia lista unica (non ancora spostate in un'annata come "2026"). Vuoi spostarle ora in una nuova annata? (I dati restano al sicuro comunque vada: "No, ignora" si limita a nascondere questo avviso, senza toccare nulla.)</span>
        <div style="display:flex;gap:8px;flex-shrink:0">
          <button class="btn btn-blu btn-sm" onclick="migraDatiAnalisi()">Migra dati esistenti</button>
          <button class="btn btn-sm" onclick="ignoraMigrazioneAnalisi()">No, ignora</button>
        </div>
      </div>`;
  } catch (e) {
...
function ignoraMigrazioneAnalisi() {
  const state = ANALISI_PREZZI_STATE;
  if (!state) return;
